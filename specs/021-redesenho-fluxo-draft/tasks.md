...
**Revalidação pós-rejeição (2026-07-26)**: T030 foi reexecutada integralmente na rota autenticada real sem `focus(selector)`, com 361 transições por Tab/Shift+Tab, 12 checkpoints após comandos, zero retorno ao `body`, zero `outlineStyle: none`, seis jornadas interativas em PT/EN, mutações de roster extenso, ações por teclado e ponteiro nos quatro viewports e artefatos reproduzíveis em `/tmp/opencode/feature021-e2e`.
