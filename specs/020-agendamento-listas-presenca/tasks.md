...
- [X] T022 Executar testes PostgreSQL e script idempotente, registrando GREEN de schema, paginação/count, timezone, concorrência, claim e rollback em `specs/020-agendamento-listas-presenca/tasks.md`. GREEN corrigido após segunda revisão em 2026-07-24: 39 testes PostgreSQL aprovados, incluindo terminais com claim nulo, janela persistida, TTL exato, constraints, publicação pelo agregado, rollback/reuso, conclusões concorrentes, configuração obsoleta, agenda pausada/arquivada, retomada de bloqueada e expiração de claim; suíte backend com 401 aprovados; build Release com 0 avisos/erros; script idempotente gerado; `has-pending-model-changes` informou ausência de drift; migration `20260723090000_AddAgendamentosPresenca` alinhada com designer/snapshot.
