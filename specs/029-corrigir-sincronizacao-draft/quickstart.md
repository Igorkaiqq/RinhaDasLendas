...
- A targeted fresh run used disposable database `rinha_feature029_browser_qa_unit13_timing` and measured from browser request initiation (`XMLHttpRequest.send`, no later than network send) to the stable canonical Vue projection. Restore `Finalizada` v18 -> v19 started at epoch `1785734627658.9` and settled in session A at `1785734627819.7` (`160.8 ms`) and session B at `1785734627798.8` (`139.9 ms`). Archive v19 -> v20 started at `1785734677481.5` and settled in A at `1785734677624.9` (`143.4 ms`) and B at `1785734677636.7` (`155.2 ms`). Each action used exactly one successful PATCH and each client applied the new canonical version once; all four nonnegative upper bounds satisfy `<= 2000 ms`.
- For recovery, both clients were isolated from the network while the backend remained healthy. `backendAvailableAt` was the first successful `/health` response after connectivity returned; the destination was the next accepted canonical GET (`canonicalAcceptanceId`) with selected version v21. Session A converged from `1785735705559.4` to `1785735706051.8` (`492.4 ms`) and session B from `1785735705588.4` to `1785735706051.4` (`463.0 ms`), both within `<= 5000 ms`. SignalR returned to `connected` later (`14537.7 ms` in A and `14514.5 ms` in B); this transport timing is reported separately and is not substituted for canonical UI convergence.
